...
# version output contains the value of PKG_SOURCE_VERSION in
# net/tollgate-wrt/Makefile (the upstream git tag; the apk-legal PKG_VERSION
# is the same number with underscores).
#
...
# cli.Version="v$(PKG_SOURCE_VERSION)" (i.e. "v0.6.0-alpha1"), so a bare
# "0.6.0" always appears in the reported version either way.
#
# Exit status: 0 = pass, 1 = fail.

BINARY="/usr/bin/tollgate"
UPSTREAM_TAG="0.6.0-alpha1"
...
if printf '%s\n' "$OUTPUT" | grep -qi "0\.6\.0"; then
    echo "PASS: $BINARY reports version $UPSTREAM_TAG"
    exit 0
fi

echo "FAIL: $BINARY did not report version $UPSTREAM_TAG" >&2
printf 'Output:\n%s\n' "$OUTPUT" >&2
exit 1
